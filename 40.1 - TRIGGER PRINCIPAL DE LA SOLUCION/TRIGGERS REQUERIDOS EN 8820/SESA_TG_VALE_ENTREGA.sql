...
CREATE TRIGGER SESA_TG_VALE_ENTREGA ON SAitemfac
--WITH ENCRYPTION
AFTER INSERT
AS
...
IF @codoper='01-301' and @tipofac='C' -- ES UNA OPERACION DE SERVICIO Y NOTA DE ENTREGA EN VENTA
 ...
 IF EXISTS (SELECT NumeroD FROM dbo.SAFACT WHERE (TipoFac = 'G' and NumeroD = @OrdenReparacion)) 
  ...
  IF @CODCLIE='99002' OR @CODCLIE='99003' OR @CODCLIE='99004'
   BEGIN
    ...
    IF SUBSTRING(@LIQUIDACION,1,1)='G' OR SUBSTRING(@LIQUIDACION,1,1)='I' OR SUBSTRING(@LIQUIDACION,1,1)='A'
     ...
     IF SUBSTRING(@STATUS,1,1)='P'
      ...
      SELECT DISTINCT X.CodItem
         ...
         WHERE (X.TipoFac = 'C' AND Y.Nro_OR = @OrdenReparacion) AND Z.NUMEROR IS NULL
      OPEN MIREG
    ...
    ELSE
      BEGIN
    SET @RESULTADO='<<ENTREGA INVALIDA>>  O/R ESTA CERRADA, NO PUEDEN CARGARSE REPUESTOS EN ELLA.  '
    SET @ELIMINAR='1'
      END
  END
  ELSE
      BEGIN
   SET @RESULTADO='<<ENTREGA INVALIDA>> LA LIQUIDACION DE LA O/R NO ES VALIDA PARA PROCESAR ESTA ACCION, REVISE LOS DATOS E INTENTE DE NUEVO'
   SET @ELIMINAR='1'
      END
  END
  ELSE
       BEGIN
   SET  @RESULTADO='<<ENTREGA INVALIDA>> CLIENTE INVALIDO, REVISE EL CODIGO DE CLIENTE E INTENTE DE NUEVO '
   SET  @ELIMINAR='1'
       END
 END 
 ELSE
       BEGIN
    SET @RESULTADO='<<ENTREGA INVALIDA>> O/R:' + @OrdenReparacion + ' NO EXISTE EN LA BASE DE DATOS, REVISE LOS DATOS E INTENTE DE NUEVO'
    SET @ELIMINAR='1'
       END
...
-- MUESTRA PANTALLA CON RESULTADO DE VALIDACIONES SI LAS HAY.
   ...
   UPDATE SAFACT SET NOTAS1=@RESULTADO, ORDENC=@OrdenReparacion WHERE NUMEROD=@NUMEROD AND TIPOFAC='C'   
