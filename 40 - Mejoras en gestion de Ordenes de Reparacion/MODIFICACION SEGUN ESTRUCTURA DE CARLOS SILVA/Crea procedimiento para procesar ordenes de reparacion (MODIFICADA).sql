...
-- 3RA Modificacion: Modificacion DE FONDO SE CREAN SAFACT_02 Y SAFACT_03 SE DEJA SOLO SAFACT_01
-- PARA GESTION DE OR 
...
DECLARE @Serial_M varchar(35)
DECLARE @Color varchar (25)
DECLARE @FechaVenta datetime
DECLARE @Concesio varchar(20)
...
BEGIN
   -- Toma los datos originales de la orden de reparacion y datos de vehiculos  y los prepara
   SELECT  @Placa       = UPPER(X.Placa),
--          @OrdenCompra = UPPER(X.Orden_de_compra),
            @Codigo      = UPPER(ISNULL(z.Modelo, '')),
            @Serial      = UPPER(z.Serial),
            @Serial_M    = UPPER(Z.SERIAL_M),
            @Color       = UPPER(z.Color),
            @FechaVenta  = z.vendido,
            @Concesio    = z.vendio_concesionario,
...
--          @Revisado    = UPPER(X.Revisado),
          ...
          @Notas9      = Y.Notas9,
          @Notas10     = Y.Notas10
   FROM  dbo.SESA_VW_ORDENES_REPARACION AS X INNER JOIN dbo.SAFACT AS Y
   ON (X.TipoFac = Y.TipoFac AND X.NumeroD = Y.NumeroD) INNER JOIN SESA_VW_VEHICULO_NUEVO_SERVICIO AS Z
...
BEGIN
   ...
   -- Actualiza orden de reparacion y vehiculos nuevos
   ...
   UPDATE dbo.SESA_VW_VEHICULO_NUEVO_SERVICIO
   SET Modelo              = @Codigo,
       Serial              = @Serial,
       Serial_M            = @Serial_M,
       Color               = @Color,
...
BEGIN
   ...
   BEGIN
     IF @COPROD = ''
        BEGIN
          RAISERROR ('El vehiculo no existe en Base de datos debe Registrarlo para continuar,16,1)
