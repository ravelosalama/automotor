-- VISTA QUE RELACIONA LOS PEDIDOS Y SU CONTENIDO CARGADOS A OR  
-- APLICACION: COMPLEMENTO SOLUCION 76 / REPORTES AGREGADOS 201 
-- REALIZADO EL:07/07/2011 POR: JOSE RAVELO

DROP VIEW dbo.SESA_VW_PEDIDOS_SOBRE_ORDENES

GO
CREATE VIEW dbo.SESA_VW_PEDIDOS_SOBRE_ORDENES
WITH ENCRYPTION
...
AS
  ...
  FROM         SAFACT_01 AS X INNER JOIN
                      SAFACT_03 AS Y ON X.NumeroD = Y.Nro_OR AND Y.TipoFac = 'E' INNER JOIN
...
declare @TNAME CHAR(29)
declare @ALIAS CHAR(29)
SET @TNAME = 'SESA_VW_PEDIDOS_SOBRE_ORDENES'
SET @ALIAS = 'SESA_VW_PEDIDOS_SOBRE_ORDENES'
